...
import { initMateSidebar, showMateSidebar, hideMateSidebar, renderMateSessionList } from './modules/mate-sidebar.js';
...
// --- DOM refs ---
  ...
  var mateResponseText = "";
  var matePendingAskUser = null;  // { toolId, questions } - pending AskUserQuestion
  ...
  function connectMateWs(slug, onReady) {
    disconnectMateWs();
    mateProjectSlug = slug;
    mateResponseText = "";
    matePendingAskUser = null;
  ...
  function handleMateWsMessage(msg) {
    if (!dmMode || !dmTargetUser || !dmTargetUser.isMate) return;

    switch (msg.type) {
      case "init":
        // Got session list from mate project
        if (msg.sessions) {
      ...
      case "text_delta":
        mateResponseText += (msg.text || "");
        break;

      case "done":
        // Finalize accumulated response as a DM message
        if (mateResponseText.trim()) {
          var responseText = mateResponseText.trim();
          // Parse [[MATE_READY: name]] marker
          var readyMatch = responseText.match(/\[\[MATE_READY:\s*(.+?)\]\]/);
          if (readyMatch && dmTargetUser) {
            var newName = readyMatch[1].trim();
            responseText = responseText.replace(/\[\[MATE_READY:\s*.+?\]\]/, "").trim();
            // Update local mate data
            dmTargetUser.displayName = newName;
            var dmHeaderName = document.getElementById("dm-header-name");
            if (dmHeaderName) dmHeaderName.textContent = newName;
            var dmInput = document.getElementById("dm-input");
            if (dmInput) dmInput.placeholder = "Message " + newName;
            // Tell server to update mate
      ...
      case "tool_executing":
        // AskUserQuestion: display question as mate DM message
        if (msg.name === "AskUserQuestion" && msg.input && msg.input.questions) {
          // First flush any accumulated text
          if (mateResponseText.trim()) {
            appendDmMessage({
              from: dmTargetUser.id,
              text: mateResponseText.trim(),
              ts: Date.now(),
            });
            mateResponseText = "";
          }
          // Display questions as a DM message
          var qTexts = [];
          var questions = msg.input.questions;
          for (var qi = 0; qi < questions.length; qi++) {
            var q = questions[qi];
            var qLine = q.question || "";
            if (q.options && q.options.length > 0) {
              var optLabels = [];
              for (var oi = 0; oi < q.options.length; oi++) {
                optLabels.push(q.options[oi].label || q.options[oi].value || q.options[oi]);
              }
              qLine += "\n" + optLabels.map(function (o, idx) { return (idx + 1) + ". " + o; }).join("\n");
            }
            qTexts.push(qLine);
          }
          appendDmMessage({
          ...
          // Store pending ask so next DM message sends the answer
          matePendingAskUser = { toolId: msg.id, questions: questions };
        }
        break;
    }
  }
  ...
  function handleDmSend() {
    ...
    if (dmTargetUser && dmTargetUser.isMate && mateWs && mateWs.readyState === 1) {
      // Show user's message in DM UI immediately
      appendDmMessage({ from: myUserId, text: text, ts: Date.now() });
      scrollToBottom();

      if (matePendingAskUser) {
        // Respond to pending AskUserQuestion
        var answers = {};
        answers[0] = text;
        mateWs.send(JSON.stringify({
          type: "ask_user_response",
          toolId: matePendingAskUser.toolId,
          answers: answers,
        }));
        matePendingAskUser = null;
      } else {
        // Send as regular query to mate project
        mateWs.send(JSON.stringify({ type: "query", text: text }));
      }
    } else {
      ...
      switch (msg.type) {
        ...
        case "dm_history":
          ...
          if (pendingMateInterview && msg.targetUser && msg.targetUser.isMate && msg.projectSlug) {
            ...
            // Wait for mateWs to connect, then send interview prompt
            var checkMateWs = setInterval(function () {
              if (mateWs && mateWs.readyState === 1) {
                clearInterval(checkMateWs);
                var interviewText = buildMateInterviewPrompt(interviewMate);
                mateWs.send(JSON.stringify({ type: "query", text: interviewText }));
                appendDmMessage({ from: myUserId, text: interviewText, ts: Date.now() });
                scrollToBottom();
              }
            }, 100);
            // Safety timeout
            setTimeout(function () { clearInterval(checkMateWs); }, 5000);
          }
          break;
