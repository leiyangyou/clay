...
function renderHome(filter) {
  ...
  if (recentSessions.length > 0) {
    ...
    for (var r = 0; r < recentSessions.length; r++) {
      ...
      if (s.isMate) {
        ...
        if (s.projectIcon && s.projectIcon.indexOf("img:") === 0) {
          iconHtml = '<img src="' + escapeHtml(s.projectIcon.substring(4)) + '" style="width:18px;height:18px;object-fit:cover;border-radius:3px;">';
        } else {
      ...
      if (s.projectIcon && s.projectIcon.indexOf("img:") === 0) {
        ...
        projIconLabel = (s.projectIcon || "") + " ";
      }
      var projLabel = (projIconLabel + escapeHtml(s.projectTitle || s.projectSlug)).trim();
  ...
  if (filteredProjects.length > 0) {
    ...
    for (var j = 0; j < filteredProjects.length; j++) {
      ...
      var projIconHtml = '<i data-lucide="box"></i>';
      if (proj.icon && proj.icon.indexOf("img:") === 0) {
        projIconHtml = '<img src="' + escapeHtml(proj.icon.substring(4)) + '" style="width:18px;height:18px;object-fit:cover;border-radius:3px;">';
      } else if (proj.icon) {
        projIconHtml = proj.icon;
      }
      html += renderItem(flatIndex, projIconHtml, pName, sessLabel, null);
      flatIndex++;
    }
  }
...
function appendHomeSearchResults(results, query) {
  ...
  for (var i = 0; i < newResults.length; i++) {
    ...
    if (r.isMate) {
      ...
      if (r.projectIcon && r.projectIcon.indexOf("img:") === 0) {
        srIconHtml = '<img src="' + escapeHtml(r.projectIcon.substring(4)) + '" style="width:18px;height:18px;object-fit:cover;border-radius:3px;">';
      } else {
    ...
    if (r.projectIcon && r.projectIcon.indexOf("img:") === 0) {
      ...
      srProjIconLabel = (r.projectIcon || "") + " ";
    }
    var projLabel = (srProjIconLabel + escapeHtml(r.projectTitle || r.projectSlug)).trim();
