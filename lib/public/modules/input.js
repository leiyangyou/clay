...
export var builtinCommands = [
  ...
  { name: "reload-plugins", desc: "Reload plugins, skills, and MCP servers" },
];
...
export function sendMessage() {
  ...
  if (text === "/reload-plugins") {
    ...
    autoResize();
    if (ctx.ws && ctx.connected) {
      ctx.addSystemMessage("Reloading plugins...", true);
      ctx.ws.send(JSON.stringify({ type: "reload_plugins" }));
    }
    return;
  }
...
export function initInput(_ctx) {
  ...
  if (inputRow) {
    inputRow.addEventListener("dragenter", function (e) {
      e.preventDefault();
      e.stopPropagation();
      dragCounter++;
      if (dragCounter === 1) {
