...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid, sessionData) {
  ...
  exportItem.className = "session-ctx-item";
  exportItem.innerHTML = iconHtml("file-text") + " <span>Export transcript</span>";
  exportItem.addEventListener("click", function (e) {
    e.stopPropagation();
    closeSessionCtxMenu();
    if (ctx.ws && ctx.connected) {
      ctx.ws.send(JSON.stringify({ type: "get_transcript_turns", id: sessionId }));
    }
  });
...
function renderSheetProjects(listEl) {
  for (var i = 0; i < cachedProjectList.length; i++) {
    (function (p) {
      ...
      if (p.icon && p.icon.indexOf("img:") === 0) {
        var mobileImg = document.createElement("img");
        mobileImg.src = p.icon.substring(4);
        mobileImg.style.width = "20px";
        mobileImg.style.height = "20px";
        mobileImg.style.objectFit = "cover";
        mobileImg.style.borderRadius = "4px";
        abbrev.appendChild(mobileImg);
      } else if (p.icon) {
  ...
  for (var ci = 0; ci < cachedProjectList.length; ci++) {
    (function (p) {
      ...
      if (p.icon && p.icon.indexOf("img:") === 0) {
        ...
        chipImg.src = p.icon.substring(4);
        chipImg.style.width = "16px";
        chipImg.style.height = "16px";
        chipImg.style.objectFit = "cover";
        chipImg.style.borderRadius = "3px";
        abbrev.appendChild(chipImg);
      } else if (p.icon) {
        abbrev.textContent = p.icon;
        parseEmojis(abbrev);
      } else {
...
// Category icons & labels (order matches emoji-mart data)
var EMOJI_CAT_META = {
...
// Hardcoded frequent/favorites for instant render before CDN data loads
var EMOJI_FREQUENT = [
  "😀","😎","🤓","🧠","💡","🔥","⚡","🚀",
  "🎯","🎮","🎨","🎵","📦","📁","📝","💻",
  "🖥️","⌨️","🔧","🛠️","⚙️","🧪","🔬","🧬",
...
// Built from emoji-mart data; starts with just frequent tab
var EMOJI_CATEGORIES = [
  { id: "frequent", icon: "🕐", label: "Frequent", emojis: EMOJI_FREQUENT },
];

// Emoji search keywords — Apple-quality keywords with emoji-mart categories
var EMOJI_KEYWORDS = {};

// Fetch emoji-mart (categories/structure) + local keyword data in parallel
(function () {
  var emojiMartUrl = "https://cdn.jsdelivr.net/npm/@emoji-mart/data@1.2.1/sets/15/native.json";
  var keywordsUrl = "data/emoji-keywords.json";

  Promise.all([
    fetch(emojiMartUrl).then(function (r) { return r.json(); }),
    fetch(keywordsUrl).then(function (r) { return r.json(); }).catch(function () { return null; }),
  ]).then(function (results) {
    var martData = results[0];
    var appleKw = results[1];
    var emojis = martData.emojis || {};
    var cats = martData.categories || [];

    // Build keyword index — prefer Apple keywords, fall back to emoji-mart
    if (appleKw) {
      EMOJI_KEYWORDS = appleKw;
    }
    // Supplement with emoji-mart keywords for any emoji not in Apple data
    var keys = Object.keys(emojis);
    for (var i = 0; i < keys.length; i++) {
      var e = emojis[keys[i]];
      if (e.skins && e.skins[0] && e.skins[0].native) {
        var native = e.skins[0].native;
        if (!EMOJI_KEYWORDS[native]) {
          EMOJI_KEYWORDS[native] = [e.name.toLowerCase()].concat(e.keywords || []).join(" ");
        }
      }
    }

    // Build categories from emoji-mart data
    var newCats = [{ id: "frequent", icon: "🕐", label: "Frequent", emojis: EMOJI_FREQUENT }];
    for (var c = 0; c < cats.length; c++) {
      var cat = cats[c];
      var meta = EMOJI_CAT_META[cat.id];
      if (!meta) continue;
      var natives = [];
      for (var j = 0; j < cat.emojis.length; j++) {
        var em = emojis[cat.emojis[j]];
        if (em && em.skins && em.skins[0] && em.skins[0].native) {
          natives.push(em.skins[0].native);
        }
      }
      newCats.push({ id: cat.id, icon: meta.icon, label: meta.label, emojis: natives });
    }
    EMOJI_CATEGORIES = newCats;
  }).catch(function () { /* fallback: frequent tab only */ });
})();
function searchEmojis(query) {
  ...
  for (var c = 0; c < EMOJI_CATEGORIES.length; c++) {
    for (var i = 0; i < EMOJI_CATEGORIES[c].emojis.length; i++) {
      allEmojis.push(EMOJI_CATEGORIES[c].emojis[i]);
    }
  }
  for (var j = 0; j < allEmojis.length; j++) {
    var emoji = allEmojis[j];
    var kws = EMOJI_KEYWORDS[emoji] || "";
    var match = true;
    for (var t = 0; t < terms.length; t++) {
      if (kws.indexOf(terms[t]) === -1 && emoji.indexOf(terms[t]) === -1) {
        match = false;
        break;
      }
    }
...
function showEmojiPicker(slug, anchorEl) {
  ...
  var sidebarUploadBtn = document.createElement("button");
  sidebarUploadBtn.className = "settings-btn-sm";
  sidebarUploadBtn.textContent = "Upload Image";
  sidebarUploadBtn.style.fontSize = "11px";
  sidebarUploadBtn.style.padding = "2px 8px";
  ...
  header.appendChild(sidebarUploadBtn);
...
document.addEventListener("click", function (e) {
  closeProjectCtxMenu();
  if (emojiPickerEl && emojiPickerEl.contains(e.target)) return;
  closeEmojiPicker();
});
...
function createMobileProjectItem(p, currentSlug, isWorktree) {
  ...
  abbrev.className = "mobile-project-abbrev";
  if (p.icon && p.icon.indexOf("img:") === 0) {
    var mobileItemImg = document.createElement("img");
    mobileItemImg.src = p.icon.substring(4);
    mobileItemImg.style.width = "20px";
    mobileItemImg.style.height = "20px";
    mobileItemImg.style.objectFit = "cover";
    mobileItemImg.style.borderRadius = "4px";
    abbrev.appendChild(mobileItemImg);
  } else if (p.icon) {
    abbrev.textContent = p.icon;
    parseEmojis(abbrev);
  } else {
