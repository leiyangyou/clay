...
function attachSessions(ctx) {
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "get_transcript_turns") {
      if (msg.id && sm.sessions.has(msg.id)) {
        var result = sm.getTranscriptTurns(msg.id);
        if (!result.error) {
          sendTo(ws, { type: "transcript_turns", id: msg.id, turns: result.turns, title: result.title, createdAt: result.createdAt });
        }
      }
      return true;
    }
    ...
    if (msg.type === "reload_plugins") {
      var session = getSessionForWs(ws);
      if (session) {
        sdk.reloadPlugins(session);
      } else {
        sendTo(ws, { type: "error", text: "No active session for plugin reload" });
      }
      return true;
    }
