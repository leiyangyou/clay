...
function createSDKBridge(opts) {
  ...
  async function startQueryViaWorker(session, text, images, linuxUser) {
    ...
    worker.onMessage(function(msg) {
      ...
      switch (msg.type) {
        ...
        case "plugins_reloaded_result":
          applyReloadResult(msg.result || {});
          break;
  ...
  // --- Reload plugins: hot-reload MCP servers, skills, agents, hooks ---

  // Shared helper: rebuild slash commands from SDK reload result and broadcast
  function applyReloadResult(result) {
    var fsSkills = discoverSkillDirs();
    var sdkCmdNames = [];
    if (Array.isArray(result.commands)) {
      for (var i = 0; i < result.commands.length; i++) {
        sdkCmdNames.push(result.commands[i].name || result.commands[i]);
      }
    }
    sm.skillNames = mergeSkills(sdkCmdNames, fsSkills);

    var seen = new Set();
    var combined = [];
    var all = sdkCmdNames.concat(Array.from(sm.skillNames));
    for (var k = 0; k < all.length; k++) {
      if (!seen.has(all[k])) {
        seen.add(all[k]);
        combined.push(all[k]);
  ...
  async function reloadPlugins(session) {
    // Worker mode: delegate only if worker has an active queryInstance.
    // Between queries queryInstance is null in the worker, so fall through
    // to the direct path which can create a temporary query instance.
    if (session.worker && session.queryInstance) {
      session.worker.send({ type: "reload_plugins" });
      return;
    }

    // Need a queryInstance to call the SDK method
    var qi = session.queryInstance;
    var tempCleanup = null;
    if (!qi) {
      // Create a temporary query instance (same pattern as rewind)
      try {
        var temp = await getOrCreateRewindQuery(session);
        qi = temp.query;
        if (temp.isTemp) tempCleanup = temp.cleanup;
      } catch (e) {
        send({ type: "error", text: "Failed to reload plugins: no active session" });
        return;
      }
    }

    try {
      var result = await qi.reloadPlugins();
      applyReloadResult(result);
    } catch (e) {
      send({ type: "error", text: "Failed to reload plugins: " + (e.message || e) });
    } finally {
