...
function createSessionManager(opts) {
  ...
  return {
    ...
    getTranscriptTurns: function (localId) {
      var session = sessions.get(localId);
      if (!session) return { error: "Session not found" };
      var history = session.history;
      var turns = [];
      ...
      for (var i = 0; i < history.length; i++) {
        ...
        if (entry.type === "user_message" || entry.type === "mention_user") {
          ...
          currentTurn = {
            ...
            hasImages: !!(entry.imageRefs && entry.imageRefs.length),
          };
        } else if ((entry.type === "delta" || entry.type === "mention_response") && entry.text && currentTurn) {
          currentTurn.assistantText += entry.text;
        }
      }
      if (currentTurn) turns.push(currentTurn);
      return { turns: turns, title: session.title || "New Session", createdAt: session.createdAt || null };
    },
