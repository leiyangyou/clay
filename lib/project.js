    ...
    if (msg.type === "mate_dm_send") {
      ...
      session.onQueryComplete = function (s) {
        ...
        if (responseText && s._mateDmKey) {
          ...
          var mateReadyMatch = responseText.match(/\[\[MATE_READY:\s*(.+?)\]\]/);
          if (mateReadyMatch && s._mateDmMateId) {
            var mateName = mateReadyMatch[1].trim();
            matesModule.updateMate(s._mateDmMateId, {
              name: mateName,
              status: "ready",
              profile: Object.assign({}, (matesModule.getMate(s._mateDmMateId) || {}).profile || {}, { displayName: mateName }),
            });
            // Strip marker from displayed text
            responseText = responseText.replace(/\[\[MATE_READY:\s*.+?\]\]/, "").trim();
            // Notify client of mate update
            if (s._mateDmWs && s._mateDmWs.readyState === 1) {
              s._mateDmWs.send(JSON.stringify({ type: "mate_updated", mate: matesModule.getMate(s._mateDmMateId) }));
            }
          }
