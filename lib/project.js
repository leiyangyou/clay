...
function createProjectContext(opts) {
  ...
  // --- Browser MCP server config ---
  // Write browser MCP server config to .claude-local/settings.json so the SDK discovers it.
  (function registerBrowserMcp() {
    if (isMate) return; // No MCP for mate projects
    try {
      var localSettingsDir = path.join(cwd, ".claude-local");
      var localSettingsPath = path.join(localSettingsDir, "settings.json");
      var existing = {};
      try {
        if (fs.existsSync(localSettingsPath)) {
          existing = JSON.parse(fs.readFileSync(localSettingsPath, "utf8"));
        }
      } catch (e) {}
      if (!existing.mcpServers) existing.mcpServers = {};
      existing.mcpServers.browser = {
        command: "node",
        args: [path.join(__dirname, "browser-mcp-server.js")],
        env: {
          CLAY_PORT: String(serverPort),
          CLAY_SLUG: slug,
          CLAY_EXT_TOKEN: _extToken,
          CLAY_TLS: serverTls ? "1" : "0",
        },
      };
      fs.mkdirSync(localSettingsDir, { recursive: true });
      fs.writeFileSync(localSettingsPath, JSON.stringify(existing, null, 2) + "\n");
    } catch (e) {
      console.log("[browser-mcp] Failed to write MCP config for " + slug + ": " + e.message);
    }
  })();
  ...
  function handleConnection(ws, wsUser) {
    ...
    // Filter out tab: sources on restore — tab IDs are ephemeral and stale after browser restart
    var restoredSources = loadContextSources(slug).filter(function(id) { return !id.startsWith("tab:"); });
    saveContextSources(slug, restoredSources); // Clean stale tab sources from disk too
