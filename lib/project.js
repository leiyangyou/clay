...
function createProjectContext(opts) {
  ...
  function sendExtensionCommand(ws, command, args) {
    return new Promise(function(resolve) {
      var requestId = crypto.randomUUID();
      var timer = setTimeout(function() {
        delete pendingExtensionRequests[requestId];
        resolve(null); // timeout after 3 seconds
      }, 3000);
  ...
  function handleConnection(ws, wsUser) {
    ...
    sendTo(ws, { type: "context_sources_state", active: loadContextSources(slug).filter(function(id) { return !id.startsWith("tab:"); }) });
  ...
  function handleMessage(ws, msg) {
    ...
    var tabSources = ctxSources.filter(function(id) { return id.startsWith("tab:"); });
